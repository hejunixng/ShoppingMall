    ...
    <div class="orderconfirm">
      ...
      <defs>
                    ...
                    <div class="item-adress">
                         ...
                         <div class="addr-list clearfix" >
                                ...
                                <div class="addr-info" :class="{checked:index == checkIndex}" @click="checkIndex = index" v-for="(item,index) in list" :key="index">
                                    ...
                                    <a href="javascript:;" class="fr" @click="fixAddress(item)" >
                                        <svg class="icon icon-edit"><use xlink:href="#icon-edit"></use></svg>
                                    </a>
                                    </div>
                                </div>
                                
                                <div class="addr-add" @click="editAddress()">
        ...
        <modal
          title="删除确认"
          btnType='1'
          :showModal='is_EditModal'
          @cancel = 'is_EditModal = false'
          @cart = 'submit()'
        >
        <template v-slot:body>
          <div class="edit-wrap">
              <div class="item">
                <input type="text" class="input" placeholder="姓名" v-model="addObj.receiverName">
                 <input type="text" class="input" placeholder="手机号" v-model="addObj.receiverMobile">
              </div>
              <div class="item">
                <select name="province" v-model="addObj.receiverProvince" >
                  <option value="北京">北京</option>
                  <option value="天津">天津</option>
                  <option value="河北">河北</option>
                </select>
                <select name="city" v-model="addObj.receiverCity">
                  <option value="北京">北京</option>
                  <option value="天津">天津</option>
                  <option value="河北">石家庄</option>
                </select>
                <select name="district" v-model="addObj.receiverDistrict">
              ...
              <div class="item">
                <textarea name="street" v-model="addObj.receiverAddress"></textarea>
              </div>
              <div class="item">
                <input type="text"  class="email" placeholder="邮编" v-model="addObj.receiverZip">
              </div>
          </div>
        </template>
        </modal>
        <modal
          title="删除确认"
          btnType='1'
          :showModal='is_showModal'
          @cancel = 'is_showModal = false'
          @cart = 'submit()'
        >
        <template v-slot:body>
          <p>您确认删除此地址吗</p>
        ...
        methods:{
            ...
            submit(){
              ...
              if(userAction == 0){
                ...
                method = 'put',
                url = `/shippings/${addObj.id}`
              }else{
                   method = 'delete',url = `/shippings/${addObj.id}`;
              };

              if(userAction == 0 || userAction == 1){
                let{receiverName,receiverMobile,receiverProvince,receiverCity,receiverDistrict,receiverAddress,receiverZip} = addObj;
                let errmsg = '';
                if(!receiverName){
                  errmsg = '请输入用户名'
            ...
            closeModal(){
              ...
              this.is_EditModal=false
            },
          //订单结算提交
          orderPay(){
            // 根据index判断是否选中地址
            let index = this.list[this.checkIndex];
            if(!index){
          ...
          console.log(res);
            ...
            getCartList(){
                this.axios.get('/carts').then((res)=>{
                    let list = res.cartProductVoList;//总商品
